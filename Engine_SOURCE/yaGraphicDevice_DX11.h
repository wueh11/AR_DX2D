...
namespace ya::graphics
{
	...
	public:
		...
		bool CreateDomainShader(const void* pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage* pClassLinkage, ID3D11DomainShader** ppDmainShader);
		...
		void BindComputeShader(ID3D11ComputeShader* pPixelShader, ID3D11ClassInstance* const* ppClassInstances, UINT NumClassInstances);
	...
	private:
		...
		// 화면에 최종적으로 그려지는 백버퍼(Frame Buffer)를 관리하고, 실제로 화면에 렌더링하는 역할을 담당하는 객체
	...
	inline GraphicDevice_DX11*& GetDevice() /// 역참조 설정
	{
