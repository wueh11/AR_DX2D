...
namespace ya
{
	...
	public:
		...
		static std::shared_ptr<T> Find(const std::wstring& key)
		{
			...
			{
				return std::dynamic_pointer_cast<T>(iter->second); /// dynamic_pointer_cast -> 포인터를 형변환할때 사용
			}
		...
		{
			mResources.insert(std::make_pair(key, dynamic_pointer_cast<Resource>(resource)));
		}
