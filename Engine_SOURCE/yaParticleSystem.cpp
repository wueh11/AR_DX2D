...
namespace ya
{
	...
	void ParticleSystem::Initialize()
	{
		...
		material->SetTexture(tex, eTextureSlot::T0);
		...
		{
			particles[i].position = Vector4(0.0f, 0.0f, 1000.0f, 1.0f);
			particles[i].active = 0;
			particles[i].direction =
				Vector4(cosf((float)i * (XM_2PI / (float)mCount)), sin((float)i * (XM_2PI / (float)mCount)), 0.0f, 1.0f);
